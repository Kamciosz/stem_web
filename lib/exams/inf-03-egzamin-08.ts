/**
 * Single source of truth dla arkusza INF.03 / styczeń 2026 / 08.
 * Dashboard + 4 podstrony etapow.
 */

export type ExamStepSlug = "baza-danych" | "html" | "php" | "css" | "kontrola";

export type ExamStepView = {
    slug: ExamStepSlug;
    index: number;
    label: string;
    short: string;
    summary: string;
    minutes: string;
    technologies: string[];
};

export type ExamStep = ExamStepView & {
    mdx: () => Promise<{ default: React.ComponentType }>;
};

export type ExamMeta = {
    courseId: "inf-03";
    lessonSlug: string;
    examId: string;
    session: string;
    title: string;
    topic: string;
    description: string;
    rule: string;
    time: string;
    technologies: string[];
    scoreTarget: string;
    scoringTotal: string;
    objective: string;
};

export const examMeta: ExamMeta = {
    courseId: "inf-03",
    lessonSlug: "egzamin-08-styczen-2026",
    examId: "INF.03-08",
    session: "Styczeń 2026",
    title: "Arkusz 08 — Korona gór polskich",
    topic: "Korona gór polskich",
    description:
        "Przewodnik po rozwiązaniu w kolejności pracy. Cztery etapy, każdy na osobnej stronie. Plan, materiały, błędy i kod — bez jednego długiego scrolla.",
    rule: "SQL → PHP → CSS → Kontrola",
    time: "150 min",
    technologies: ["PHP", "SQL", "CSS", "HTML"],
    scoreTarget: "~22 / 30 pkt",
    scoringTotal: "30 pkt",
    objective: "Wykonanie aplikacji internetowej portalu korony gór polskich",
};

export const examSteps: ExamStep[] = [
    {
        slug: "baza-danych",
        index: 1,
        label: "Baza danych",
        short: "Baza",
        summary: "Szczyty, opisy, LIKE, LIMIT i JOIN z opisem szczytu.",
        minutes: "0–25 min",
        technologies: ["SQL", "MySQL"],
        mdx: () => import("@/content/inf-03/egzamin-08-styczen-2026/baza-danych.mdx"),
    },
    {
        slug: "html",
        index: 2,
        label: "HTML",
        short: "HTML",
        summary: "Index i szczyty.php, nagłówki, miniatury, stopki i linki do szczegółów.",
        minutes: "25–45 min",
        technologies: ["HTML"],
        mdx: () => import("@/content/inf-03/egzamin-08-styczen-2026/html.mdx"),
    },
    {
        slug: "php",
        index: 3,
        label: "PHP",
        short: "PHP",
        summary: "Lista szczytów, miniatury i szczegóły pobierane przez parametr GET.",
        minutes: "45–90 min",
        technologies: ["PHP", "MySQL"],
        mdx: () => import("@/content/inf-03/egzamin-08-styczen-2026/php.mdx"),
    },
    {
        slug: "css",
        index: 4,
        label: "CSS",
        short: "CSS",
        summary: "@font-face, overflow miniatur, nth-child i układ Korony Gór Polskich.",
        minutes: "90–130 min",
        technologies: ["CSS"],
        mdx: () => import("@/content/inf-03/egzamin-08-styczen-2026/css.mdx"),
    },
    {
        slug: "kontrola",
        index: 5,
        label: "Kontrola",
        short: "Kontrola",
        summary: "Weryfikacja wymagań, punktacja, checklista.",
        minutes: "130–150 min",
        technologies: ["QA"],
        mdx: () => import("@/content/inf-03/egzamin-08-styczen-2026/kontrola.mdx"),
    },
];


export const examStrategy = [
    { time: "0–25 min", title: "Baza / analiza", body: "Rozpisz dane wejściowe, sprawdź zapytania SQL albo wymagane pliki.", tag: "łatwe punkty" },
    { time: "25–90 min", title: "HTML i PHP", body: "Struktura HTML, logika skryptów i dane wejściowe zgodnie z arkuszem.", tag: "główna część" },
    { time: "90–130 min", title: "CSS pod makietę", body: "Odtwarzaj polecenie: fonty, kolory, układ, obrazy i formularze.", tag: "dopięcie" },
    { time: "130–150 min", title: "Kontrola", body: "Porównaj wymagania, działanie i wygląd punkt po punkcie.", tag: "ostatnie punkty" },
];

export function getStepBySlug(slug: string): ExamStep | undefined {
    return examSteps.find((s) => s.slug === slug);
}

export const examStepsView: ExamStepView[] = examSteps.map(({ mdx: _mdx, ...rest }) => rest);

export function toExamStepView(step: ExamStep): ExamStepView {
    const { mdx: _mdx, ...view } = step;
    return view;
}

/** Materialy z arkusza — dashboard pokazuje realne pliki arkusza. */
export const examMaterials = {
    files: [
        { src: "/img/egzaminy/e08-logo.png", alt: "e08-logo.png — materiał arkusza INF.03-08", title: "e08-logo.png", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-babiagora.jpg", alt: "e08-babiagora.jpg — materiał arkusza INF.03-08", title: "e08-babiagora.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-rysy.jpg", alt: "e08-rysy.jpg — materiał arkusza INF.03-08", title: "e08-rysy.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-sniezka.jpg", alt: "e08-sniezka.jpg — materiał arkusza INF.03-08", title: "e08-sniezka.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-tarnica.jpg", alt: "e08-tarnica.jpg — materiał arkusza INF.03-08", title: "e08-tarnica.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-turbacz.jpg", alt: "e08-turbacz.jpg — materiał arkusza INF.03-08", title: "e08-turbacz.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-wysoka.jpg", alt: "e08-wysoka.jpg — materiał arkusza INF.03-08", title: "e08-wysoka.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-sleza.jpg", alt: "e08-sleza.jpg — materiał arkusza INF.03-08", title: "e08-sleza.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-szczeliniec.jpg", alt: "e08-szczeliniec.jpg — materiał arkusza INF.03-08", title: "e08-szczeliniec.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-mogielica.jpg", alt: "e08-mogielica.jpg — materiał arkusza INF.03-08", title: "e08-mogielica.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-lackowa.jpg", alt: "e08-lackowa.jpg — materiał arkusza INF.03-08", title: "e08-lackowa.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-gora.jpg", alt: "e08-gora.jpg — materiał arkusza INF.03-08", title: "e08-gora.jpg", caption: "materiał z arkusza" },
        { src: "/img/egzaminy/e08-gora2.jpg", alt: "e08-gora2.jpg — materiał arkusza INF.03-08", title: "e08-gora2.jpg", caption: "materiał z arkusza" },
    ],
    result: {
        src: "/img/egzaminy/e08-kw4.png",
        alt: "Makieta lub wynik końcowy INF.03-08 — Korona gór polskich",
        title: "e08-kw4.png",
        caption: "porównaj z własnym rozwiązaniem",
    },
} as const;

/** Pelna checklista — uzywana na podstronie Kontrola i jako summary na dashboardzie. */
export const examChecklistKeys = [
    "sql-zapytania",
    "php-polaczenie",
    "utf8",
    "petle",
    "formularz-post",
    "obliczenia-lub-insert",
    "obrazy-sciezki",
    "css-makieta",
    "brak-warningow",
    "makieta-zgodna",
] as const;

export const examFlowBasePath = `/kursy/${examMeta.courseId}/${examMeta.lessonSlug}`;
